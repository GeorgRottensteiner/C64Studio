...
namespace Be.Windows.Forms
{
	...
	public class HexBox : Control
	{
		...
		class KeyInterpreter : IKeyInterpreter
		{
			...
			{
        if ( _hexBox.StringViewActive && _hexBox._keyInterpreter.GetType() == typeof( KeyInterpreter ) )
				{
		...
		#region Fields
    /// <summary>
    /// returns true if the common string view is visible and should react to mouse/keys
    /// </summary>
    bool StringViewActive
    {
      get
      {
        return ( ( _stringViewVisible )
        &&       ( _customHexViewer == null ) );
      }
    }
    ...
    /// <summary>
    /// Contains the hex data view bounds
    /// </summary>
    Rectangle _recHexDataView;

    /// <summary>
    /// Font for string view
    /// </summary>
    Font _TextFont;
    ...
    /// </summary>
    [Description( "Occurs, when the view is scrolled." )]
    ...
    /// <summary>
		...
		/// <summary>
		/// Occurs, when the value of CustomHexViewer property has changed.
		/// </summary>
    [Description( "Occurs, when the value of CustomHexViewer property has changed." )]
    public event EventHandler CustomHexViewerChanged;
		/// <summary>
      ...
      OnViewScrolled( EventArgs.Empty );
		}
    ...
    public long VScrollPos
    {
      ...
      if ( _customHexViewer != null )
      {
		...
		void PaintLineInfo(Graphics g, long startByte, long endByte)
		{
			...
			for (int i = 0; i < maxLine; i++)
			{
				...
				int nulls = 4 - info.Length;
				string formattedInfo;
				if (nulls > -1)
				{
					formattedInfo = new string('0', 4 - info.Length) + info;
				}
				else
				{
					formattedInfo = new string('~', 4);
				}
      ...
      Brush brushMarked = new SolidBrush( GetMarkedForeColor() );
        ...
        bool isMarked = ( ( _selectedByteProvider != null )
                        && ( _selectedByteProvider.IsByteSelected( i ) ) );
				...
				{
          PaintHexString( g, b, isMarked ? brushMarked : brush, gridPoint );
				}
			}
		}
      ...
      Brush brushMarked = new SolidBrush( GetMarkedForeColor() );
        ...
        bool isMarked = ( ( _selectedByteProvider != null )
          ...
          g.DrawString( s, TextFont, selBrush, byteStringPointF, _stringFormat );
				}
				else
				{
          g.DrawString( s, TextFont, brush, byteStringPointF, _stringFormat );
				}
			}
		}
    ...
    Color GetMarkedForeColor()
    {
      ...
      if ( _customHexViewer != null )
      {
        _recHexDataView = new Rectangle( _recHex.X + _recHex.Width,
                                         _recHex.Y,
                                         _recContent.Width - ( _recHex.X + _recHex.Width ),
                                         _recHex.Height );
      }
      else
      {
        _recHexDataView = Rectangle.Empty;
      }
    ...
    public ISelectedByteProvider SelectedByteProvider
    {
      get
      {
        return _selectedByteProvider;
      }
      set
      {
        _selectedByteProvider = value;
        Invalidate();
      }
    }
		/// <summary>
    ...
    ISelectedByteProvider _selectedByteProvider;
    ...
    /// <summary>
    /// Gets or sets the custom hex data viewer
    /// </summary>
    [DefaultValue( false ), Category( "Hex" ), Description( "Gets or sets the custom hex data viewer" )]
    public ICustomHexViewer CustomHexViewer
    {
      get { return _customHexViewer; }
      set
      {
        if ( _customHexViewer == value )
          return;

        _customHexViewer = value;
        OnCustomHexViewerChanged( EventArgs.Empty );

        UpdateRectanglePositioning();
        Invalidate();
      }
    } ICustomHexViewer _customHexViewer;
    ...
    /// <summary>
    /// Raises the CustomHexViewerChanged event.
    /// </summary>
    /// <param name="e">An EventArgs that contains the event data.</param>
    protected virtual void OnCustomHexViewerChanged( EventArgs e )
    {
      if ( CustomHexViewerChanged != null )
        CustomHexViewerChanged( this, e );
    }
