...
namespace C64Studio.Parser
{
  public class ASMFileParser : ParserBase
  {
    ...
    public bool ParseValue( int LineIndex, string Value, out SymbolInfo ResultingSymbol, out int NumGivenBytes )
    {
      ...
      if ( ParseLiteralValue( Value, out failed, out int IntegerValue, out NumGivenBytes ) )
      {
    ...
    private SymbolInfo CreateIntegerSymbol( int Value )
    {
      int     dummy;
    ...
    private SymbolInfo CreateIntegerSymbol( int Value, out int NumBytesGiven )
    {
      var symbol = new SymbolInfo();
    ...
    private List<Types.TokenInfo> ProcessExtFunction( int LineIndex, string FunctionName, List<Types.TokenInfo> Tokens, int StartIndex, int Count )
    {
    ...
    private bool EvaluateExtFunction( int LineIndex, string FunctionName, List<Types.TokenInfo> Tokens, int StartIndex, int Count, out int Result )
    {
      Result = 0;
    ...
    private bool EvaluateExtFunctionNumeric( int LineIndex, string FunctionName, List<Types.TokenInfo> Tokens, int StartIndex, int Count, out double Result )
    {
      Result = 0;
          ...
          case TokenInfo.TokenType.LITERAL_NUMBER:
            {
              ...
              ParseLiteralValue( Tokens[StartIndex].Content, out bool failed, out symbol.AddressOrValue, out NumBytesGiven );
              symbol.Type = SymbolInfo.Types.CONSTANT_1;

              int result = symbol.AddressOrValue;
              result &= 0xffff;
              if ( result > 255 )
              {
                symbol.Type = SymbolInfo.Types.CONSTANT_2;
      ...
      do
      {
        ...
        &&   ( bracketEndPos != -1 ) )
        {
          ...
          if ( bracketStartPos > 0 )
          {
            ...
            {
              // handle function!
              List<Types.TokenInfo> results = ProcessExtFunction( LineIndex, possibleFunction, subTokenRange,  bracketStartPos + 1, bracketEndPos - bracketStartPos - 1 );
              if ( ( results == null )
              ||   ( results.Count != 1 ) )
              {
                return false;
        ...
        if ( Count >= 2 )
        {
          ...
          if ( highestPrecedence != -1 )
          {
            ...
            &&   ( subTokenRange[highestPrecedenceTokenIndex + 1].Content == "-" ) )
            {
              ...
              int     negatedResult = -value.AddressOrValue;
                ...
                case MacroInfo.PseudoOpType.FILL:
                  {
                    ...
                    if ( !EvaluateTokens( lineIndex, lineInfo.NeededParsedExpression, 0, tokenCommaIndex, out SymbolInfo symbol , out dummyBytesGiven ) )
                    {
                      AddError( lineIndex,
                    ...
                    if ( !EvaluateTokens( lineIndex, lineInfo.NeededParsedExpression, tokenCommaIndex + 1, lineInfo.NeededParsedExpression.Count - tokenCommaIndex - 1, out symbol ) )
                    {
              ...
              if ( lineInfo.Opcode != null )
              {
                ...
                { 
                  ...
                  else
                  {
                    ...
                    {
                      int     zeroPageValue = zeroPageValueSymbol.ToInteger( );
                      int     relativeValue = relativeValueSymbol.ToInteger( );
                      // zeropage numerand
                ...
                {
                  int delta = addressValue - lineInfo.AddressStart - 2;
                  if ( ( delta < -128 )
                  ||   ( delta > 127 ) )
                  {
                    AddError( lineIndex, Types.ErrorCode.E1100_RELATIVE_JUMP_TOO_FAR, "Relative jump too far, trying to jump " + delta + " bytes" );
    ...
    private void PODataByte( int LineIndex, List<Types.TokenInfo> lineTokenInfos, int StartIndex, int Count, Types.ASM.LineInfo info, Types.MacroInfo.PseudoOpType Type, GR.Collections.Map<byte, byte> TextMapping, bool AllowNeededExpression )
    {
      ...
      for ( int tokenIndex = StartIndex; tokenIndex < StartIndex + Count; ++tokenIndex )
      {
        ...
        if ( token == "," )
        {
          ...
          if ( tokenIndex - firstTokenIndex >= 1 )
          {
            ...
            if ( EvaluateTokens( LineIndex, lineTokenInfos, firstTokenIndex, tokenIndex - firstTokenIndex, out SymbolInfo byteValueSymbol, out numBytesGiven ) )
            {
              int   byteValue = byteValueSymbol.ToInteger();
              switch ( Type )
              {
      ...
      if ( firstTokenIndex + 1 <= lineTokenInfos.Count )
      {
        ...
        if ( EvaluateTokens( LineIndex, lineTokenInfos, firstTokenIndex, lineTokenInfos.Count - firstTokenIndex, out SymbolInfo byteValueSymbol, out numBytesGiven ) )
        {
          int byteValue = byteValueSymbol.ToInteger( );
          switch ( Type )
          {
                                 ...
                                 ref String[] Lines )
    {
      ...
      else if ( lastOpenedScope.RepeatUntil != null )
      {
        ...
        if ( repeatUntil.Content == null )
        {
          ...
          repeatUntil.Content = RelabelLocalLabelsForLoop( repeatUntil.Content, ScopeList, lineIndex );
        }
        ...
        int result = resultSymbol.ToInteger( );
    ...
    private bool POIncludeMedia( List<Types.TokenInfo> lineTokenInfos, int lineIndex, bool Binary, Types.ASM.LineInfo info, string ParentFilename, out int lineSizeInBytes, out string[] ReplacementLines )
    {
      ...
      if ( extension == ".CHR" )
      {
        ...
        {
          startIndex = startIndexSymbol.ToInteger();
        }
        if ( ( paramTokens.Count >= 4 )
        &&   ( EvaluateTokens( lineIndex, paramTokens[3], out SymbolInfo numCharsSymbol ) ) )
        {
          numChars = numCharsSymbol.ToInteger();
        }
        try
        {
      ...
      else if ( extension == ".VALUETABLEPROJECT" )
      {
        ...
        {
          numBytes = numBytesSymbol.ToInteger();
        }
      ...
      else if ( extension == ".SPR" )
      {
        ...
        if ( method == "SPRITE" )
        {
          ...
          {
            numSprites = numSpritesSymbol.ToInteger();
          }
          try
          {
            dataToInclude = GR.IO.File.ReadAllBytes( subFilename );
        ...
        else if ( method == "SPRITEDATA" )
        {
          ...
          {
            offsetBytes = offsetBytesSymbol.ToInteger();
          }
      ...
      else if ( extension == ".SPRITEPROJECT" )
      {
        ...
        ||   ( method == "PALETTESWIZZLED" ) )
        {
          ...
          {
            numColors = numColorsSymbol.ToInteger();
          }
          if ( ( startIndex < 0 )
    ...
    private ParseLineResult POLoopStart( List<Types.TokenInfo> lineTokenInfos, int lineIndex, Types.ASM.LineInfo info, ref string[] Lines, List<Types.ScopeInfo> Scopes, out int lineSizeInBytes )
    {
      ...
      else
      {
        ...
        {
          numLoops = numLoopsSymbol.ToInteger();
          bool hadError = false;
          if ( numLoops <= 0 )
          {
    ...
    private ParseLineResult POBasic( string Line, List<Types.TokenInfo> lineTokenInfos, int lineIndex, Types.ASM.LineInfo info, GR.Collections.Map<byte,byte> textCodeMapping, bool AllowLaterEvaluation, out int lineSizeInBytes )
    {
      ...
      *=$2001
      ...
      &&        ( poParams.Count == 1 ) )
      {
        ...
        jumpAddress = jumpAddressSymbol.ToInteger();
        if ( ( jumpAddress < 0 )
        ||   ( jumpAddress >= 65536 ) )
        {
      ...
      &&        ( realNumParams == 2 ) )
      {
        ...
        basicLineNumber = basicLineNumberSymbol.ToInteger();
    ...
    private ParseLineResult POAlignDASM( List<Types.TokenInfo> lineTokenInfos, int lineIndex, Types.ASM.LineInfo info, ref int programStepPos, out int lineSizeInBytes )
    {
      ...
      do
      {
        if ( lineTokenInfos[tokenIndex].Content == "," )
        {
          ...
          tokenParams.Add( value.ToInteger() );
          expressionStartIndex = tokenIndex + 1;
        }
        ++tokenIndex;
    ...
    private ParseLineResult PODataWord( List<Types.TokenInfo> lineTokenInfos, int LineIndex, int StartIndex, int Count, Types.ASM.LineInfo info, String parseLine, bool AllowNeededExpression, bool LittleEndian, out int lineSizeInBytes )
    {
      ...
      for ( int tokenIndex = StartIndex; tokenIndex < StartIndex + Count; ++tokenIndex )
      {
        ...
        if ( token == "," )
        {
          ...
          if ( tokenIndex - firstTokenIndex >= 1 )
          {
            ...
            if ( EvaluateTokens( LineIndex, lineTokenInfos, firstTokenIndex, tokenIndex - firstTokenIndex, out SymbolInfo wordValueSymbol, out numBytesGiven ) )
            {
              wordValue = wordValueSymbol.ToInteger();
      ...
      if ( firstTokenIndex + 1 <= lineTokenInfos.Count )
      {
        ...
        if ( EvaluateTokens( LineIndex, lineTokenInfos, firstTokenIndex, lineTokenInfos.Count - firstTokenIndex, out SymbolInfo wordValueSymbol, out numBytesGiven ) )
        {
          wordValue = wordValueSymbol.ToInteger();
          if ( !ValidWordValue( wordValue ) )
          {
    ...
    private ParseLineResult POIncludeBinary( List<Types.TokenInfo> lineTokenInfos, int lineIndex, Types.ASM.LineInfo info, out int lineSizeInBytes )
    {
      ...
      if ( fileSizeValid )
      {
        fileSize = fileSizeSymbol.ToInteger();
      }
      if ( fileSkipValid )
      {
        fileSkip = fileSkipSymbol.ToInteger();
      }
      ...
      recheck_line:;
        ...
        else if ( IsDefine( lineTokenInfos ) )
        {
          ...
          if ( defineName == "*" )
          {
            ...
            if ( !EvaluateTokens( lineIndex, tokens, out SymbolInfo newStepPosSymbol ) )
            {
          ...
          else
          {
            ...
            if ( !EvaluateTokens( lineIndex, valueTokens, out SymbolInfo addressSymbol ) )
            {
        ...
        &&   ( IsOpcode( lineTokenInfos[0].Type ) ) )
        {
          ...
          if ( info.Opcode != null )
          {
            ...
            else if ( info.Opcode.NumOperands == 1 )
            {
              ...
              if ( EvaluateTokens( lineIndex, lineTokenInfos, 1, lineTokenInfos.Count - 1, out SymbolInfo byteValueSymbol ) )
              {
            ...
            else if ( info.Opcode.NumOperands == 2 )
            {
              ...
              if ( info.Opcode.Addressing == Tiny64.Opcode.AddressingType.ZEROPAGE_RELATIVE )
              {
                ...
                else
                {
                  ...
                  int     relativeValue;
                  if ( ( !EvaluateTokens( lineIndex, tokenInfos[0], out SymbolInfo zeroPageValueSymbol ) )
                  ||   ( !EvaluateTokens( lineIndex, tokenInfos[1], out SymbolInfo relativeValueSymbol ) ) )
                  {
                    info.NeededParsedExpression = lineTokenInfos.GetRange( 1, countTokens );
                  }
                  else
                  {
                    zeroPageValue = zeroPageValueSymbol.ToInteger();
                    relativeValue = relativeValueSymbol.ToInteger();
                    // zeropage numerand
                    if ( !ValidByteValue( zeroPageValue ) )
                    {
                      AddError( lineIndex,
              ...
              else if ( EvaluateTokens( lineIndex, lineTokenInfos, startIndex, countTokens, out SymbolInfo byteValueSymbol ) )
              {
                byteValue = byteValueSymbol.ToInteger();
          ...
          else
          {
            ...
            else if ( pseudoOp.Type == Types.MacroInfo.PseudoOpType.BANK )
            {
              ...
              else
              {
                ...
                {
                  number = numberSymbol.ToInteger();
                  size = sizeSymbol.ToInteger();
        ...
        &&   ( m_AssemblerSettings.PseudoOps.ContainsKey( upToken ) ) )
        {
          ...
          else if ( macroInfo.Type == C64Studio.Types.MacroInfo.PseudoOpType.ORG )
          {
            ...
            if ( commaCount == 1 )
            {
              ...
              newStepPos = newStepPosSymbol.ToInteger();
              newPseudoPos = newPseudoPosSymbol.ToInteger();
            }
            else
            {
    ...
    private ParseLineResult PORepeat( List<TokenInfo> lineTokenInfos, int lineIndex, ref string[] Lines, LineInfo info, List<ScopeInfo> Scopes )
    {
      ...
      if ( !EvaluateTokens( lineIndex, lineParams[0], out SymbolInfo numRepeatsSymbol ) )
      {
        return ParseLineResult.ERROR_ABORT;
      }
      int numRepeats = numRepeatsSymbol.ToInteger( );
      if ( ( numRepeats < 0 )
      ||   ( numRepeats > 99999 ) )
      {
        AddError( lineIndex,
    ...
    private ParseLineResult POPseudoPC( Types.ASM.LineInfo info, List<Types.ScopeInfo> Scopes, int lineIndex, List<Types.TokenInfo> lineTokenInfos, int TokenStartIndex, int TokenCount )
    {
      ...
      info.PseudoPCOffset = pseudoStepPos.ToInteger();
      return ParseLineResult.OK;
    }
    ...
    private void POFor( List<Types.ScopeInfo> stackScopes, string zoneName, ref int intermediateLineOffset, int lineIndex, List<Types.TokenInfo> lineTokenInfos )
    {
      ...
      else
      {
        ...
        else
        {
          ...
          if ( indexStep != -1 )
          {
            if ( !EvaluateTokens( lineIndex, lineTokenInfos, indexStep + 1, lineTokenInfos.Count - indexStep - 1, out SymbolInfo stepValueSymbol ) )
            {
              AddError( lineIndex,
                        C64Studio.Types.ErrorCode.E1302_MALFORMED_MACRO,
                        "Malformed macro, expect !FOR <Variable> = <Start Value Expression> TO <End Value Expression [STEP] <Step Value Expression>",
            ...
            {
              stepValue = stepValueSymbol.ToInteger();
              if ( stepValue == 0 )
              {
                AddError( lineIndex,
                          C64Studio.Types.ErrorCode.E1302_MALFORMED_MACRO,
          ...
          if ( !hadError )
          {
            int   startValue = 0;
            int   endValue = 0;
            if ( !EvaluateTokens( lineIndex, lineTokenInfos, 3, indexTo - 3, out SymbolInfo startValueSymbol ) )
            {
              AddError( lineIndex,
                        C64Studio.Types.ErrorCode.E1302_MALFORMED_MACRO,
            ...
            else if ( !EvaluateTokens( lineIndex, lineTokenInfos, indexTo + 1, indexStep - indexTo - 1, out SymbolInfo endValueSymbol ) )
            {
              AddError( lineIndex,
                        C64Studio.Types.ErrorCode.E1302_MALFORMED_MACRO,
            ...
            {
              startValue = startValueSymbol.ToInteger();
              endValue = endValueSymbol.ToInteger();
              if ( ( stepValue < 0 )
              &&   ( endValue >= startValue ) )
              {
                AddError( lineIndex,
