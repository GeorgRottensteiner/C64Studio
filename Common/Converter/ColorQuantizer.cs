...
namespace RetroDevStudio.Converter
{
  public class ColorQuantizer
  {
    ...
    GR.Image.MemoryImage Assignment( CubeInfo cube_info, GR.Image.IImage pCD )
    {
      ...
      var       pal = new GR.Image.Palette( m_Colors );
      ...
      for ( int y = 0; y < pCD.Height; y++ )
      {
        ...
        for ( int x = 0; x < pCD.Width; x += count )
        {
          ...
          if ( ( pCD.PixelFormat & GR.Drawing.PixelFormat.Indexed ) != 0 )
          {
            pixel = (uint)( pCD.PaletteRed( (int)pixel ) << 16 ) + (uint)( pCD.PaletteGreen( (int)pixel ) << 8 ) + pCD.PaletteBlue( (int)pixel );
          }
