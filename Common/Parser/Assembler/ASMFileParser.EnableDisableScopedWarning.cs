...
namespace RetroDevStudio.Parser
{
  ...
  {
    private ParseLineResult POEnableDisableWarning( List<TokenInfo> TokenInfos, bool enable )
    {
      if ( !ParseLineInParameters( TokenInfos, 1, TokenInfos.Count - 1, _ParseContext.LineIndex, false, out List<List<TokenInfo>> lineParams ) )
      {
      ...
      foreach ( var warning in lineParams )
      {
        ...
        &&     ( warning[0].Content[0] != 'w' ) ) )
        {
          AddError( _ParseContext.LineIndex, Types.ErrorCode.E1000_SYNTAX_ERROR, "Malformed warning number, expect warning value" );
        ...
        foreach ( var warningEnum in warningEnums )
        {
          ...
          if ( warningText == warningEnum.Substring( 0, 5 ) )
          {
            ...
            {
              AddError( _ParseContext.LineIndex, Types.ErrorCode.E1000_SYNTAX_ERROR, $"Malformed warning number, {warning[0].Content} is not a known warning code" );
              return ParseLineResult.RETURN_NULL;
            }
            m_ScopedWarningStates.Add( new ScopedWarningState()
            {
              Warning         = actualWarning,
              GlobalLineIndex = _ParseContext.LineIndex,
              Enabled         = enable
            } );
            foundWarning = true;
            break;
