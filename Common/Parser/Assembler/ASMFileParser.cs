  ...
  public partial class ASMFileParser : ParserBase
  {
                                            ...
                                            ref String[] Lines )
    {
      ...
      else if ( lastOpenedScope.RepeatUntil != null )
      {
        ...
        if ( repeatUntil.Content == null )
        {
          ...
          // fix up public labels for first loop
      ...
      else if ( lastOpenedScope.Loop != null )
      {
        ...
        if ( ( lineTokenInfos.Last().Content == "}" ) != lastLoop.IsUsingBrackets )
        {
          AddError( lineIndex,
                    RetroDevStudio.Types.ErrorCode.E1000_SYNTAX_ERROR,
                    "Mismatching scope end! Either use {, } or !for and !end, not both",
      ...
      if ( InitialFileInfo != null )
      {
        foreach ( var entry in InitialFileInfo.Labels )
        {
          ...
          &&   ( entry.Value.Type != SymbolInfo.Types.PREPROCESSOR_LABEL ) )
          {
            ...
            {
              // do not pass on public local labels
              continue;
            }
            var symbol = new SymbolInfo();
    ...
    private string[] PreProcess( string[] Lines, string ParentFilename, ProjectConfig Configuration, string AdditionalPredefines, out bool HadFatalError )
    {
      ...
      for ( int lineIndex = 0; lineIndex < Lines.Length; ++lineIndex )
      {
        ...
        // identify public labels as such
    ...
    public List<Types.TokenInfo> ParseTokenInfo( string Source, int Start, int Length )
    {
      ...
      // if the last token is an public label (+/-), and something useless is following, it is interpreted as operator
      // why tf?
      /*
      if ( ( result.Count >= 2 )
      ...
      // macro'd public labels turn up as global label, operator, global label, collapse into one token
      ...
      if ( result.Count > 1 )
      {
        for ( int i = 0; i < result.Count - 1; ++i )
        {
          ...
          // public label in front
          if ( ( result[i].Type == TokenInfo.TokenType.OPERATOR )
          &&   ( ( result[i].Content.StartsWith( "+" ) )
          ||     ( result[i].Content.StartsWith( "-" ) ) ) )
          {
            // is directly connected to global label then the public label is split
            if ( ( result.Count > 1 )
        ...
        // opcode followed by public label?
      ...
      // public label followed by global starting with public local prefix (replaced public label in macro call)
      for ( int i = 0; i < result.Count; ++i )
      {
      ...
      // public label followed by global is a macro call 
    ...
    public string TokensToExpression( List<Types.TokenInfo> Tokens )
    {
    ...
    public string TokensToExpression( List<Types.TokenInfo> Tokens, int StartIndex, int Count )
    {
      StringBuilder sb = new StringBuilder();
