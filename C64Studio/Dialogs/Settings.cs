...
namespace RetroDevStudio.Dialogs
{
    public partial class Settings : Form
    {
        ...
        private void btnBrowseTool_Click(object sender, EventArgs e)
        {
            ...
            if (dlgTool.ShowDialog() == DialogResult.OK)
            {
                ...
                if (tool.Filename != labelToolPath.Text)
                {
                    ...
                    && (string.IsNullOrEmpty(tool.TapeArguments))
                    && (string.IsNullOrEmpty(tool.DiskArguments))
                    && (tool.AdditionalParameters == null || tool.AdditionalParameters.Count <1))
                    {
        ...
        private void editToolTapeArguments_TextChanged(object sender, EventArgs e)
        {
            var tool = SelectedTool();
            if (tool == null) return;
            tool.TapeArguments = editToolTapeArguments.Text;
        }

        private void editToolDiskArguments_TextChanged(object sender, EventArgs e)
        {
            var tool = SelectedTool();
            if (tool == null) return;
            tool.DiskArguments = editToolDiskArguments.Text;
        }
        ...
        private void editToolPRGArguments_TextChanged(object sender, EventArgs e)
        {
            ...
            tool.PrgArguments = editToolProgramArguments.Text;
        }
            ...
            foreach (var xmlKey in xmlSettingRoot.ChildElements)
            {
                if (xmlKey.Type == "Tool")
                {
                    try
                    {
                        ...
                        toolInfo.TapeArguments = xmlKey.Attribute("TapeArgs");
                        toolInfo.DiskArguments = xmlKey.Attribute("DiskArgs");
                        ...
                        var additionalParams = xmlKey.FindByTypeRecursive("AdditionalParameters");

                        if (additionalParams != null && additionalParams.GetChildCount() > 0)
                        {
                            foreach (GR.Strings.XMLElement param in additionalParams.ChildElements)
                            {
                                if (param.Type != "AdditionalParameter") continue;

                                var newParam = new ToolInfoRuntimeParameter(
                                    param.Attribute("Name"),
                                    param.Attribute("Value"),
                                    GetBooleanFromString(param.Attribute("Enabled")));
                                toolInfo.AdditionalParameters.Add(newParam);
                            }
                        }
        ...
        private void ExportTools(GR.Strings.XMLElement XMLRoot)
        {
            ...
            foreach (ToolInfo tool in Core.Settings.ToolInfos)
            {
                ...
                xmlKey.AddAttribute("TapeArgs", tool.TapeArguments);
                xmlKey.AddAttribute("DiskArgs", tool.DiskArguments);
                xmlKey.AddAttribute("WorkPath", tool.WorkPath);

                var xmlParams = new GR.Strings.XMLElement("AdditionalParameters");

                foreach (var param in tool.AdditionalParameters)
                {
                    var xmlParam = new GR.Strings.XMLElement("AdditionalParameter");
                    xmlParam.AddAttribute(nameof(param.Name), param.Name);
                    xmlParam.AddAttribute(nameof(param.Value), param.Value);
                    xmlParam.AddAttribute(nameof(param.Enabled), param.Enabled ? "yes" : "no");
                    xmlParams.AddChild(xmlParam);
                }

                xmlKey.AddChild(xmlParams);
        ...
        private ArrangedItemEntry alistTools_CloningItem(object sender, ArrangedItemEntry Item)
        {
            ...
            tool.TapeArguments = origTool.TapeArguments;
            tool.DiskArguments = origTool.DiskArguments;
            tool.Type = origTool.Type;

            if(origTool.AdditionalParameters != null && origTool.AdditionalParameters.Count >0)
            {
                foreach(var arg in origTool.AdditionalParameters)
                {
                    tool.AdditionalParameters.Add(new ToolInfoRuntimeParameter(arg.Name, arg.Value));
                }
            }
        ...
        private void alistTools_SelectedIndexChanged(object sender, ArrangedItemEntry Item)
        {
            ...
            if (alistTools.SelectedItem == null)
            {
                editToolProgramArguments.Enabled = false;
                editToolTapeArguments.Enabled = false;
                editToolDiskArguments.Enabled = false;
            ...
            editToolProgramArguments.Enabled = true;
            editToolTapeArguments.Enabled = true;
            editToolDiskArguments.Enabled = true;
            ...
            editToolProgramArguments.Text = tool.PrgArguments;
            editToolTapeArguments.Text = tool.TapeArguments;
            editToolDiskArguments.Text = tool.DiskArguments;
            ...
            DgvArguments.DataSource = new BindingList<ToolInfoRuntimeParameter>(tool.AdditionalParameters);
        ...
        {
            if (IsMakingChanges) return;
            btnAdd.Enabled = alistTools.SelectedItem != null;
            var selectedRow = DgvArguments.SelectedCells.Count > 0 ? DgvArguments.Rows[DgvArguments.SelectedCells[0].RowIndex] : null;
            var selectedItem = selectedRow != null ? selectedRow?.DataBoundItem as ToolInfoRuntimeParameter : null;
            btnClone.Enabled = alistTools.SelectedItem != null && selectedItem != null;
            btnDelete.Enabled = alistTools.SelectedItem != null && selectedItem != null;
            var tool = alistTools.SelectedItem != null ? alistTools.Items[alistTools.SelectedIndex].Tag as ToolInfo : null;
            btnMoveUp.Enabled = selectedItem != null && tool != null && tool?.AdditionalParameters.IndexOf(selectedItem) > 0;
            btnMoveDown.Enabled = selectedItem != null && tool != null && tool?.AdditionalParameters.IndexOf(selectedItem) < tool?.AdditionalParameters.Count - 1;
        }

        private bool IsMakingChanges;

        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (alistTools.SelectedItem == null) return;
            var tool = alistTools.SelectedItem != null ? alistTools.Items[alistTools.SelectedIndex].Tag as ToolInfo : null;
            if (tool == null) return;
            IsMakingChanges = true;
            DgvArguments.DataSource = null;
            tool.AdditionalParameters.Add(new ToolInfoRuntimeParameter());
            IsMakingChanges = false;
            DgvArguments.DataSource = new BindingList<ToolInfoRuntimeParameter>(tool.AdditionalParameters);
            DgvArguments.Rows[DgvArguments.Rows.Count - 1].Cells[0].Selected = true;
        }
        ...
        private void btnClone_Click(object sender, EventArgs e)
        {
            ...
            IsMakingChanges = true;
            DgvArguments.DataSource = null;
            tool.AdditionalParameters.Add(new ToolInfoRuntimeParameter($"{selectedItem.Name} - Copy", selectedItem.Value, selectedItem.Enabled));
            IsMakingChanges = false;
            DgvArguments.DataSource = new BindingList<ToolInfoRuntimeParameter>(tool.AdditionalParameters);
            DgvArguments.Rows[DgvArguments.Rows.Count-1].Cells[0].Selected = true;
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            IsMakingChanges = true;
            DgvArguments.DataSource = null;
            tool.AdditionalParameters.Remove(selectedItem);
            IsMakingChanges = false;
            DgvArguments.DataSource = new BindingList<ToolInfoRuntimeParameter>(tool.AdditionalParameters);
        }
        ...
        private void btnMoveUp_Click(object sender, EventArgs e)
        {
            ...
            var selectedItem = selectedRow != null ? selectedRow?.DataBoundItem as ToolInfoRuntimeParameter : null;
            if (selectedItem == null) return;
            var index = tool.AdditionalParameters.IndexOf(selectedItem);
            if (index <= 0) return;
            IsMakingChanges = true;
            DgvArguments.DataSource = null;
            tool.AdditionalParameters.RemoveAt(index);
            tool.AdditionalParameters.Insert(--index, selectedItem);
            IsMakingChanges = false;
            DgvArguments.DataSource = new BindingList<ToolInfoRuntimeParameter>(tool.AdditionalParameters);
            DgvArguments.Rows[index].Cells[0].Selected = true;
        }
        ...
        private void btnMoveDown_Click(object sender, EventArgs e)
        {
            ...
            if (selectedItem == null) return;
            var index = tool.AdditionalParameters.IndexOf(selectedItem);
            if (index >= tool.AdditionalParameters.Count-1) return;
            IsMakingChanges = true;
            DgvArguments.DataSource = null;
            tool.AdditionalParameters.RemoveAt(index);
            tool.AdditionalParameters.Insert(++index, selectedItem);
