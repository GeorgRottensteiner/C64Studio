...
{
  public class DeniseRemoteDebuggerBinaryInterface : DebuggerBase, IDebugger
  {
    ...
    public DeniseRemoteDebuggerBinaryInterface( StudioCore Core ) : base( Core, false, "" )
    {
    }
    ...
    public bool ConnectToEmulator( bool IsCartridge, string externalImageToOpen )
    {
      ...
      RequestData initialBreakpointRequest = null;
      try
      {
        if ( ( _initialBreakpoints.Count > 0 )
        &&   ( _initialBreakpoints[0].Breakpoint != null )
        &&   ( _initialBreakpoints[0].Breakpoint.Initial ) )
        {
      ...
      if ( client.Connected )
      {
        ...
        if ( initialBreakpointRequest != null )
        {
          QueueRequest( initialBreakpointRequest );
        }
      }
    ...
    void Connected( IAsyncResult iar )
    {
      ...
      try
      {
        ...
        Core.AddToOutput( "Connected to Denise binary interface successfully" + Environment.NewLine );
        client.BeginReceive( data, 0, size, System.Net.Sockets.SocketFlags.None, new AsyncCallback( ReceiveData ), client );
      }
      catch ( System.Net.Sockets.SocketException se )
      {
        Core.AddToOutput( "Connecting to Denise binary interface failed: " + se.ToString() );
      }
    ...
    private bool HandleBinaryInterface()
    {
      ...
      while ( true )
      {
        ...
        if ( errorCode != 0 )
        {
          Log( $"Error {errorCode:X2} received for request {requestID}, skipped command" );
          if ( errorCode == 0x80 )
          {
        ...
        switch ( responseType )
        {
          ...
          case BinaryMonitorCommandResponse.MON_RESPONSE_CHECKPOINT_INFO:
            {
              ...
              Debug.Log( $"Breakpoint at address ${startAddress:X2} has ID {checkPointNumber}, enabled {enabled}, temporary {temporary}, currentlyHit {currentlyHit}, _initialStartupState{_initialStartupState}" );

              // initial breakpoint is set up now
          ...
          case BinaryMonitorCommandResponse.MON_RESPONSE_STOPPED:
            m_Request = new RequestData( DebugRequestType.NONE );
            // initial breakpoint was hit and we stopped, other breakpoints have been added, run again
    ...
    void ReceiveData( IAsyncResult iar )
    {
      ...
      catch ( System.Net.Sockets.SocketException se )
      {
        ...
        Core.AddToOutputLine( "Connection to Denise was closed" );
        /*
        DebugEvent( new DebugEventData()
        {
          Type = RetroDevStudio.DebugEvent.EMULATOR_CLOSED
    ...
    public bool SendCommand( GR.Memory.ByteBuffer Command )
    {
      ...
      {
        Core.AddToOutput( "SendCommand Exception:" + se.ToString() );
        return false;
      }
    ...
    private byte ProcessorFlagsToByte( string Flags )
    {
      byte    result = 0;
