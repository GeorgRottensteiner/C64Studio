...
{
  public class D81 : MediaFormat
  {
    public const int TRACK_HEADER = 40;
    public const int SECTOR_HEADER = 0;

    public const int TRACK_BAM = 40;
    public const int SECTOR_BAM = 1;

    public const int TRACK_DIRECTORY = 40;
    public const int SECTOR_DIRECTORY = 3;

    const int BYTES_PER_DIR_ENTRY = 32;
    ...
    public ushort DiskID
    {
      get
      {
        ...
        return trackHeader.Sectors[0].Data.UInt16At( 0xa2 );
      }
      set
      {
        Track trackHeader = Tracks[TRACK_HEADER - 1];

        trackHeader.Sectors[0].Data.SetU16At( 0xa2, value );
      }
    }
    ...
    public ushort DOSType
    {
      get
      {
        ...
        return trackHeader.Sectors[0].Data.UInt16At( 0xa5 );
      }
      set
      {
        Track trackHeader = Tracks[TRACK_HEADER - 1];

        trackHeader.Sectors[0].Data.SetU16At( 0xa5, value );
      }
    }
    ...
    public GR.Memory.ByteBuffer DiskName
    {
      get
      {
        ...
        {
          _LastError = "Missing track 40";
          return null;
        }

        Track trackHeader = Tracks[TRACK_HEADER - 1];

        return trackHeader.Sectors[0].Data.SubBuffer( 0x90, 16 );
      }
      set
      {
        Track trackHeader = Tracks[TRACK_HEADER - 1];
        ...
        if ( value.Length >= 16 )
        {
          value.CopyTo( trackHeader.Sectors[0].Data, 0x90, 16 );
        }
        else
        {
          value.CopyTo( trackHeader.Sectors[0].Data, 0x90, (int)value.Length );
        }
      }
    }
    ...
    public byte DiskDOSVersion
    {
      set
      {
        ...
        trackHeader.Sectors[0].Data.SetU8At( 2, value );
      }
      get
      {
        Track trackHeader = Tracks[TRACK_HEADER - 1];

        return trackHeader.Sectors[0].Data.ByteAt( 2 );
      }
    }
    ...
    void CreateBAM()
    {
      ...
      track40.Sectors[SECTOR_HEADER].Data.SetU8At( 0, TRACK_DIRECTORY );
      track40.Sectors[SECTOR_HEADER].Data.SetU8At( 1, SECTOR_DIRECTORY );
      ...
      sectorBAM.Data.SetU8At( 0, TRACK_BAM );    
      sectorBAM.Data.SetU8At( 1, SECTOR_BAM + 1 );
    ...
    public bool IsSectorAllocated( int Track, int Sector )
    {
      _LastError = "";
    ...
    bool AddDirectoryEntry( GR.Memory.ByteBuffer Filename, int StartTrack, int StartSector, int SectorsWritten, C64Studio.Types.FileType Type )
    {
      ...
      byte    dirTrackIndex = TRACK_DIRECTORY;
