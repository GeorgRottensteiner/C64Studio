...
namespace GR.Image
{
  public class FastImage : IDisposable
  {
    ...
    public void DrawToHDC( IntPtr HDC, System.Drawing.Rectangle Rect )
    {
      ...
      StretchBlt( HDC, Rect.Left, Rect.Top, Rect.Width, Rect.Height, imageDC, 0, 0, m_Width, m_Height, TernaryRasterOperations.SRCCOPY );
    ...
    public void Draw( IntPtr Handle, System.Drawing.Rectangle Rect )
    {
      ...
      if ( m_Palette != IntPtr.Zero )
      {
        SelectPalette( dc, m_Palette, false );
        RealizePalette( dc );
      }
            ...
            case 1:
              {
                ...
                for ( int j = 0; j < Math.Abs( iHeight ); j++ )
                {
                  for ( int i = 0; i < ( iWidth + 7 ) / 8; i++ )
                  {
                    ...
                    {
                      ImageData.SetPixelData( i, Math.Abs( iHeight ) - j - 1, pData[i + j * iLO] );
                    }
                  }
                }
            ...
            case 16:
              {
                ...
                for ( j = 0; j < Math.Abs( iHeight ); j++ )
                {
                  for ( i = 0; i < iWidth; i++ )
                  {
                    ...
                    {
                      ImageData.SetPixelData( i, Math.Abs( iHeight ) - j - 1, wDummy );
                    }
                  }
                }
              }
            ...
            case 24:
              {
                ...
                for ( j = 0; j < Math.Abs( iHeight ); j++ )
                {
                  for ( i = 0; i < iWidth; i++ )
                  {
                    uint    pixel = (uint)( ( pData[i * 3 + j * iLO + 0] << 16 ) + ( pData[i * 3 + j * iLO + 1] << 8 ) + pData[i * 3 + j * iLO + 2] );
                    if ( iHeight < 0 )
                    {
                    ...
                    {
                      ImageData.SetPixelData( i, Math.Abs( iHeight ) - j - 1, pixel );
                    }
                  }
                }
              }
            ...
            case 32:
              {
                ...
                for ( j = 0; j < Math.Abs( iHeight ); j++ )
                {
                  for ( i = 0; i < iWidth; i++ )
                  {
                    ...
                    {
                      ImageData.SetPixelData( i, Math.Abs( iHeight ) - j - 1, dwDummy );
                    }
                  }
                }
              }
    ...
    public void DrawTo( GR.Image.FastImage TargetImage, int X, int Y )
    {
      ...
      int copyWidth   = Math.Min( Width, TargetImage.Width );
      int copyHeight  = Math.Min( Height, TargetImage.Height );
      ...
      else
      {
        ...
        for ( int i = 0; i < copyWidth; ++i )
        {
          ...
          {
            TargetImage.SetPixel( X + i, Y + j, GetPixel( i, j ) );
          }
        }
      }
    }
