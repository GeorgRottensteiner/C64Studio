      ...
      debugToolStripMenuItem.Visible = false;
        ...
        public bool RunCompiledFile(DocumentInfo Document, Types.CompileTargetType TargetType)
        {
            try
            {
                ...
                if (ASMFileParser.IsCartridge(TargetType))
                {
                    runArguments = toolRun.CartArguments;
                }

                if (ASMFileParser.IsTape(TargetType))
                {
                    runArguments = toolRun.TapeArguments;
                }

                if (ASMFileParser.IsDisk(TargetType))
                {
                    runArguments = toolRun.DiskArguments;
                }
                ...
                if(toolRun.AdditionalParameters.Count > 0)
                {
                    foreach(var param in toolRun.AdditionalParameters)
                    {
                        if (!param.Enabled) continue;
                        runArguments += $" {param.Value}";
                    }
                }

                if ((Document != null) && (Document.ASMFileInfo != null) && toolRun.PassLabelsToEmulator)
                {
