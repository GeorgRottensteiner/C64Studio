...
namespace RetroDevStudio.Parser
{
  public class BinaryDisassembler
  {
    ...
    public BinaryDisassembler( Processor Processor )
    {
    ...
    public string DisassembleBinary( GR.Memory.ByteBuffer Data, int DataStartAddress, int ExportStartAddress, int Length, DisassemblerSettings Settings, Map<int, int> lineAddresses, ref int localLineIndex )
    {
      ...
      while ( Length >= wrapSize )
      {
        if ( !firstLine )
        {
          AddLineAddress( localLineIndex, lineAddresses, Settings, sb, ExportStartAddress );
        }
        firstLine = false;
    ...
    private void AddLineAddress( int localLineIndex, Map<int, int> lineAddresses, DisassemblerSettings Settings, StringBuilder sb, int trueAddress )
    {
    ...
    public bool Disassemble( int DataStartAddress, 
                             ...
                             out GR.Collections.Map<int,int> lineAddresses,
                             out int FirstLineWithOpcode )
    {
      StringBuilder sb = new StringBuilder();
      Disassembly = "";
      FirstLineWithOpcode = 1;
      lineAddresses = new GR.Collections.Map<int, int>();
      ...
      while ( trueAddress < DataStartAddress + m_SourceData.Length )
      {
        ...
        ||   ( NamedLabels.Any( nl => nl.Key == trueAddress + 1 ) ) )
        {
          if ( hadBytes )
          {
            AddLineAddress( localLineIndex, lineAddresses, Settings, sb, hadBytesStart );

            sb.Append( DisassembleBinary( m_SourceData, DataStartAddress, hadBytesStart, trueAddress - hadBytesStart, Settings, lineAddresses, ref localLineIndex ) );
            hadBytes = false;
          }
          ...
          {
            AddLineAddress( localLineIndex, lineAddresses, Settings, sb, trueAddress );
