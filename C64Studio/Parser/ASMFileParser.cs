...
namespace C64Studio.Parser
{
  public class ASMFileParser : ParserBase
  {
    ...
    public ASMFileParser()
    {
      ...
      m_Processor = Systems.CPUSystem.Create6510System();
          ...
          else
          {
            ...
            else
            {
              ...
              if ( lineInfo.Opcode != null )
              {
                ...
                ||   ( lineInfo.Opcode.Addressing == Types.ASM.Opcode.AddressingType.ZEROPAGE_X )
                ||   ( lineInfo.Opcode.Addressing == Types.ASM.Opcode.AddressingType.ZEROPAGE_Y ) )
                {
                  if ( ( value < 0 )
                  ||   ( value > 255 ) )
                  {
                ...
                if ( lineInfo.Opcode.Addressing == Types.ASM.Opcode.AddressingType.RELATIVE )
                {
    ...
    private ParseLineResult PODataWord( List<Types.TokenInfo> lineTokenInfos, int LineIndex, int StartIndex, int Count, Types.ASM.LineInfo info, String parseLine, bool AllowNeededExpression, out int lineSizeInBytes )
    {
      ...
      for ( int tokenIndex = 1; tokenIndex < lineTokenInfos.Count; ++tokenIndex )
      {
        ...
        else if ( !parseFailed )
        {
          ...
          int aByte = -1;
          if ( ParseValue( lineIndex, token, out aByte ) )
          {
            if ( ( aByte < 0 )
            ||   ( aByte > 65535 ) )
            {
              AddError( lineIndex, Types.ErrorCode.E1003_VALUE_OUT_OF_BOUNDS_WORD, "Value out of bounds for word, needs to be >= 0 and <= 65535" );
            }
    ...
    private string[] PreProcess( string[] Lines, string ParentFilename, ProjectConfig Configuration )
    {
      ...
      for ( int lineIndex = 0; lineIndex < Lines.Length; ++lineIndex )
      {
        ...
        &&               ( !upToken.StartsWith( m_AssemblerSettings.MacroPrefix ) ) ) ) ) )
        {
          ...
          if ( upToken == "}" )
          {
            ...
            else
            {
              ...
              /*
              if ( ScopeInsideMacroDefinition( stackDefineBlocks ) )
              {
        ...
        {
          List<Types.ASM.Opcode> possibleOpcodes = new List<C64Studio.Types.ASM.Opcode>( m_Processor.Opcodes[upToken.ToLower()] );
          ...
          if ( lineTokenInfos[0].Type == C64Studio.Types.TokenInfo.TokenType.OPCODE_FIXED_NON_ZP )
          {
            ...
            for ( int i = 0; i < possibleOpcodes.Count; ++i )
            {
              ...
              if ( ( opcode.Addressing == C64Studio.Types.ASM.Opcode.AddressingType.ZEROPAGE )
              ||   ( opcode.Addressing == C64Studio.Types.ASM.Opcode.AddressingType.ZEROPAGE_X )
              ||   ( opcode.Addressing == C64Studio.Types.ASM.Opcode.AddressingType.ZEROPAGE_Y ) )
              {
          ...
          else if ( lineTokenInfos[0].Type == C64Studio.Types.TokenInfo.TokenType.OPCODE_DIRECT_VALUE )
          {
            ...
            for ( int i = 0; i < possibleOpcodes.Count; ++i )
            {
              ...
              if ( opcode.Addressing != C64Studio.Types.ASM.Opcode.AddressingType.IMMEDIATE )
              {
                possibleOpcodes.RemoveAt( i );
                --i;
                continue;
          ...
          info.Line = parseLine;
          Types.ASM.Opcode estimatedOpcode = EstimateOpcode( lineIndex, lineTokenInfos, possibleOpcodes, ref info );
          ...
          if ( info.Opcode != null )
          {
            ...
            else if ( info.Opcode.NumOperands == 1 )
            {
              ...
              // strip prefixed #
              if ( info.Opcode.Addressing == C64Studio.Types.ASM.Opcode.AddressingType.IMMEDIATE )
              {
              ...
              if ( EvaluateTokens( lineIndex, lineTokenInfos, 1, lineTokenInfos.Count - 1, out byteValue ) )
              {
                ...
                else if ( info.Opcode.Addressing == Types.ASM.Opcode.AddressingType.IMMEDIATE )
                {
                  if ( ( byteValue < 0 )
                  ||   ( byteValue > 255 ) )
                  {
            ...
            else if ( info.Opcode.NumOperands == 2 )
            {
              ...
              if ( ( info.Opcode.Addressing == C64Studio.Types.ASM.Opcode.AddressingType.ABSOLUTE_X )
              ||   ( info.Opcode.Addressing == C64Studio.Types.ASM.Opcode.AddressingType.ABSOLUTE_Y )
              ||   ( info.Opcode.Addressing == C64Studio.Types.ASM.Opcode.AddressingType.ZEROPAGE_X )
              ||   ( info.Opcode.Addressing == C64Studio.Types.ASM.Opcode.AddressingType.ZEROPAGE_Y )
              ||   ( info.Opcode.Addressing == C64Studio.Types.ASM.Opcode.AddressingType.INDIRECT_X )
    ...
    public override bool Assemble( CompileConfig Config )
    {
      ...
      else if ( Config.TargetType == Types.CompileTargetType.TAP )
      {
        ...
        tap.WriteFile( Util.ToFilename( "HURZ" ), Assembly, C64Studio.Types.FileType.PRG );
        /*
        Formats.T64.FileRecord  record = new C64Studio.Formats.T64.FileRecord();
      ...
      else if ( Config.TargetType == Types.CompileTargetType.D64 )
      {
        ...
        d64.WriteFile( bufName, Assembly, C64Studio.Types.FileType.PRG );

        Assembly = d64.Compile();
      }
      ...
      {
        if ( Assembly.Length < 8192 )
        {
          // fill up
          Assembly = Assembly + new GR.Memory.ByteBuffer( 8192 - Assembly.Length );
        }
        else if ( Assembly.Length > 8192 )
        {
          AddError( 0, Types.ErrorCode.E1102_PROGRAM_TOO_LARGE, "Assembly too large, " + Assembly.Length.ToString() + " > 8192" );
          return false;
        }
        if ( Config.TargetType == Types.CompileTargetType.CARTRIDGE_8K_CRT )
        {
          ...
          uint length = 16 + Assembly.Length;
      ...
      {
        if ( Assembly.Length < 16384 )
        {
          // fill up
          Assembly = Assembly + new GR.Memory.ByteBuffer( 16384 - Assembly.Length );
        }
        else if ( Assembly.Length > 16384 )
        {
          AddError( 0, Types.ErrorCode.E1102_PROGRAM_TOO_LARGE, "Assembly too large, " + Assembly.Length.ToString() + " > 16384" );
          return false;
        }
      ...
      {
        if ( Assembly.Length < 65536 )
        {
          // fill up
          Assembly = Assembly + new GR.Memory.ByteBuffer( 65536 - Assembly.Length );
        }
        else if ( Assembly.Length > 65536 )
        {
          AddError( 0, Types.ErrorCode.E1102_PROGRAM_TOO_LARGE, "Assembly too large, " + Assembly.Length.ToString() + " > 65536" );
          return false;
        }
        if ( Config.TargetType == Types.CompileTargetType.CARTRIDGE_MAGICDESK_CRT )
        {
          ...
          for ( int i = 0; i < 8; ++i )
          {
            ...
            chip.Append( Assembly.SubBuffer( i * 0x2000, 0x2000 ) );
      ...
      {
        if ( Assembly.Length < 524288 )
        {
          // fill up
          Assembly = Assembly + new GR.Memory.ByteBuffer( 524288 - Assembly.Length );
        }
        else if ( Assembly.Length > 524288 )
        {
          AddError( 0, Types.ErrorCode.E1102_PROGRAM_TOO_LARGE, "Assembly too large, " + Assembly.Length.ToString() + " > 524288" );
          return false;
        }
    ...
    public List<Types.TokenInfo> ParseTokenInfo( string Source, int Start, int Length, out ErrorInfo Error )
    {
      Error = null;
      ...
      while ( charPos < Start + Length )
      {
        ...
        if ( currentTokenType == Types.TokenInfo.TokenType.OPERATOR )
        {
          ...
          {
            Error = new ErrorInfo( -1, charPos, 1, C64Studio.Types.ErrorCode.E1000_SYNTAX_ERROR );
            result.Clear();
            return result;
          }
          continue;
          ...
          else
          {
            ...
            if ( possibleOperators == 0 )
            {
              if ( charPos > tokenStartPos )
              {
                ...
                Error = new ErrorInfo( -1, tokenStartPos, charPos - tokenStartPos, C64Studio.Types.ErrorCode.E1000_SYNTAX_ERROR );
                return null;
              }
              foreach ( KeyValuePair<Types.TokenInfo.TokenType, string> pair in m_AssemblerSettings.AllowedTokenStartChars )
              {
    ...
    private Types.ASM.Opcode EstimateOpcode( int LineIndex, List<Types.TokenInfo> LineTokens, List<Types.ASM.Opcode> PossibleOpcodes, ref Types.ASM.LineInfo info )
    {
      ...
      &&   ( PossibleOpcodes[0].Addressing == Types.ASM.Opcode.AddressingType.IMMEDIATE ) )
      {
        // that one is given
        ...
        else
        {
          ...
          ||   ( PossibleOpcodes[0].Mnemonic == "rol" ) )
          {
            ...
            &&   ( LineTokens[1].Content.ToUpper() == "A" ) )
            {
              ...
              foreach ( Types.ASM.Opcode opcode in PossibleOpcodes )
              {
                if ( opcode.Addressing == C64Studio.Types.ASM.Opcode.AddressingType.IMPLICIT )
                {
                  return opcode;
                }
              }
      ...
      Types.ASM.Opcode.AddressingType addressing = Types.ASM.Opcode.AddressingType.UNKNOWN;
      ...
      if ( LineTokens.Count == 1 )
      {
        addressing = Types.ASM.Opcode.AddressingType.IMPLICIT;
      }
      else if ( ( LineTokens.Count >= 2 )
      &&        ( LineTokens[1].Content.StartsWith( "#" ) ) )
      {
        addressing = Types.ASM.Opcode.AddressingType.IMMEDIATE;
      ...
      &&        ( !twoParamsInBrackets ) )
      {
        if ( endsWithCommaX )
        {
          ...
          {
            addressing = Types.ASM.Opcode.AddressingType.ZEROPAGE_X;
          }
          else
          {
            addressing = Types.ASM.Opcode.AddressingType.ABSOLUTE_X;
          }
        }
        else if ( endsWithCommaY )
        {
          if ( numBytesFirstParam == 1 )
          {
            addressing = Types.ASM.Opcode.AddressingType.ZEROPAGE_Y;
          }
          else
          {
            addressing = Types.ASM.Opcode.AddressingType.ABSOLUTE_Y;
          }
        }
        else
        {
          if ( numBytesFirstParam == 1 )
          { 
            addressing = Types.ASM.Opcode.AddressingType.ZEROPAGE;
          }
          else if ( numBytesFirstParam == 2 )
          {
            if ( ( PossibleOpcodes.Count > 1 )
            &&   ( addressing == Types.ASM.Opcode.AddressingType.UNKNOWN ) )
            {
              addressing = Types.ASM.Opcode.AddressingType.ABSOLUTE;
            }
          }
          else if ( addressing == Types.ASM.Opcode.AddressingType.UNKNOWN )
          {
            addressing = Types.ASM.Opcode.AddressingType.ABSOLUTE;
          }
        }
      ...
      else if ( oneParamInBrackets )
      {
        ...
        {
          addressing = Types.ASM.Opcode.AddressingType.INDIRECT_Y;
        }
        else
        {
          addressing = Types.ASM.Opcode.AddressingType.INDIRECT;
        }
      }
      else if ( twoParamsInBrackets )
      {
        addressing = Types.ASM.Opcode.AddressingType.INDIRECT_X;
      }
      else
      {
        addressing = Types.ASM.Opcode.AddressingType.IMPLICIT;
      ...
      if ( addressing == Types.ASM.Opcode.AddressingType.ZEROPAGE_X )
      {
        // no zeropage kind found, try full
        addressing = Types.ASM.Opcode.AddressingType.ABSOLUTE_X;
      }
      if ( addressing == Types.ASM.Opcode.AddressingType.ZEROPAGE_Y )
      {
        // no zeropage kind found, try full
        addressing = Types.ASM.Opcode.AddressingType.ABSOLUTE_Y;
      }
      if ( addressing == Types.ASM.Opcode.AddressingType.ZEROPAGE )
      {
        // no zeropage kind found, try full
        addressing = Types.ASM.Opcode.AddressingType.ABSOLUTE;
      }
      // was in braces, could be simple braces around
      if ( addressing == Types.ASM.Opcode.AddressingType.INDIRECT )
      {
