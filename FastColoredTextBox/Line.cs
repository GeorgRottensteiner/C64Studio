...
namespace FastColoredTextBoxNS
{
  ...
  public class Line : IList<Char>
  {
    ...
    internal void StripTrailingSpaces()
    {
      while ( ( chars.Count > 0 )
      &&      ( chars[chars.Count - 1].c == ' ' ) )
      {
    ...
    internal Line( int uid, FastColoredTextBox TextBox )
    {
      this.UniqueId = uid;
    ...
    public void AddTabified( string Text )
    {
      ...
      foreach ( char c in Text )
      {
        if ( c == '\t' )
        {
          int spaceCountNextTabStop = TextBox.TabLength - ( curPos % TextBox.TabLength );
          if ( spaceCountNextTabStop == 0 )
          {
            spaceCountNextTabStop = TextBox.TabLength;
          }

          for ( int i = 0; i < spaceCountNextTabStop; i++ )
          {
            Insert( curPos, new Char( '\t' ) );
          }
          curPos += spaceCountNextTabStop;
        }
        else
        {
    ...
    public string ToText( int Offset )
    {
